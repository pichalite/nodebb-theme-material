$(document).ready(function() {
    ...
    */
    ...
    $('.popup-chat-minimize').click(function(e){
        e.preventDefault();
    ...
    $(window).on('action:ajaxify.end', function() {
        $('#popup-chat-btn').on('click', function() {
            if ($('.popup-chat').hasClass('hidden')) {
                $('.popup-chat').toggleClass('hidden');  
            }

            var uname = $('.account-username').html();

            if(!$('.chat-user-' + ajaxify.data.theirid).length) {
                 $.getJSON(config.relative_path + '/api/user/' + utils.slugify(uname), function(user) {
                
                    $('.popup-chat-content>.nav.nav-tabs').append('<li class="chat-user-' + ajaxify.data.theirid + ' active" data-uid="' + ajaxify.data.theirid + '"><a href="#chat-user-' + ajaxify.data.theirid +'" data-toggle="tab" title="' + uname + '"><img src="' + user.picture + '"/></a></li>');
                });

                $('.popup-chat-content>.tab-content').append('<div class="tab-pane active" id="chat-user-' + ajaxify.data.theirid + '"><span class="user-typing hide"><i class="fa fa-pencil"></i> <span class="text"></span></span></div>');

                $('.popup-chat-content>.tab-content .user-typing .text').translateText('[[modules:chat.user_typing, ' + uname + ']]');
            }
        });
    });
